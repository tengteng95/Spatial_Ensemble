...
from spatial_ensemble.utils.log import setup_logger
from spatial_ensemble.utils import adjust_learning_rate_iter, save_checkpoint, parse_devices, AvgMeter
from spatial_ensemble.utils.torch_dist import configure_nccl, synchronize
...
parser.add_argument("--perserving_prob", default=0.7, type=float, help="hyperparam for spatial ensemble")

args = parser.parse_args()
if not args.exp_file:
    from spatial_ensemble.exps.sts import sts_exp
    exp = sts_exp.Exp()
else:
...
def main_worker(gpu, nr_gpu, args):
    ...
    model = exp.get_model(perserving_prob=args.perserving_prob)
