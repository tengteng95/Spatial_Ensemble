...
class M2Teacher(Module):
    def __init__(self, param_momentum, total_iters, perserving_prob):
        """
            perserving_prob: hyperparam for spatial ensemble, larger value results in lower replacing frequency and higher proportion of preserved units. 
        """
        super(M2Teacher, self).__init__()
        self.total_iters = total_iters
        self.param_momentum = param_momentum
        self.current_train_iter = 0
        self.perserving_rate = perserving_prob

        self.student_encoder = resnet.resnet50(
            low_dim=256, width=1, hidden_dim=4096, MLP="byol", CLS=False, bn="torchsync", predictor=True
        )
        self.teacher_encoder = resnet.resnet50(
            low_dim=256, width=1, hidden_dim=4096, MLP="byol", CLS=False, bn="torchsync", predictor=False
        )
