...
import os
import torch
from spatial_ensemble.models.m2_teacher import M2Teacher
import torch.distributed as dist

from spatial_ensemble.exps.sts import base_exp
...
class Exp(base_exp.BaseExp):
    ...
    def get_model(self, perserving_prob):
        if "model" not in self.__dict__:
            self.model = M2Teacher(self.param_momentum, len(self.data_loader["train"]) * self.max_epoch, perserving_prob=perserving_prob)
        return self.model

    def get_data_loader(self, batch_size, is_distributed):
        if "data_loader" not in self.__dict__:
            from spatial_ensemble.data.transforms import byol_transform
            from spatial_ensemble.data.dataset import SSL_Dataset
